...
function calcMomo({ current, past }) {
    ...
    let momo = close === pastClose ? 0 : parseFloat((((close - pastClose) / pastClose) * 100).toFixed(4));
...
function momoTrend({ momentum, close, timeframe, timestamp }) {
    ...
    let smallerTrend = smallMomentumAllPositive ? "up" : smallMomentumAllNegative ? "down" : "range";
    ...
    let biggerTrend = largeMomentumAllPositive ? "up" : largeMomentumAllNegative ? "down" : "range";
