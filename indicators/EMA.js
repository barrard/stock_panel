...
function addEMAVals(data) {
    let { tickMinutes, dailyData, fiveMinData, fifteenMinData, thirtyMinData, hourlyData } = data;
...
function EMA_Analysis(OHLC_data) {
    ...
    emaValues.forEach((emaPeriod) => {
        ...
        distanceFromPriceData[emaPeriod] = maxDiffFromPrice(emaData[emaPeriod], OHLC_data);
    });

    //then evaluate the results
    let { finalResults, filteredResults } = analyzeEMA(emaData, distanceFromPriceData, OHLC_data);

    //Doesnt really do anything right now
    let { closerLook, timeGroupedEvents, sortedCloserLook } = backTestWithResults({ finalResults, filteredResults });
...
function backTestWithResults({ finalResults, filteredResults }) {
    ...
    for (let emaPeriod in finalResults) {
        ...
        for (let percDiff in finalResults[emaPeriod]) {
            ...
            for (let results in finalResults[emaPeriod][percDiff]) {
                ...
                // console.log(results);
                if (!closerLook[emaPeriod][percDiff][results]) closerLook[emaPeriod][percDiff][results] = [];
                closerLook[emaPeriod][percDiff][results] = [...xy, ...closerLook[emaPeriod][percDiff][results]];
    ...
    timeGroupedEvents = Object.keys(timeGroupedEvents)
        ...
        .sort(
            (a, b) => new Date(b.day).toLocaleString().split(", ")[0] - new Date(a.day).toLocaleString().split(", ")[0]
        );
...
function analyzeEMA(emaData, distanceFromPriceData, OHLC_data) {
    ...
    for (let emaPeriod in emaData) {
        ...
        let limit = 4;
        filteredResults[emaPeriod] = percDiffCount.sort((a, b) => b.percAmount - a.percAmount);
        // .filter((data, i) => {
        ...
        filteredResults[emaPeriod].forEach((data) => {
            ...
            timesWhenPercDiff.forEach((xy) => {
                let cameFrom_goTo_firstGo = windowAssessment(xy, OHLC_data, percDiffArray, emaData, emaPeriod);
...
function windowAssessment({ x, y }, OHLC_data, percDiffArray, emaData, emaPeriod) {
    ...
    // slice the array
    if (timeIndex < windowLength || timeIndex + windowLength >= OHLC_data.length) return;
    // let window = OHLC_data.slice(
    //   timeIndex - windowLength,
    //   timeIndex + windowLength
    // );
    let percValuesWindow = percDiffArray.slice(timeIndex - windowLength / 2, timeIndex + windowLength / 2);
    ...
    let { cameFrom, goTo, firstGo } = whereDidYouComeFromWhereDidYouGo(percValuesWindow, emaDataWindow, emaPeriod);
...
function windowEmaData(emaData, timeIndex, windowLength) {
    ...
    Object.keys(emaData).forEach((emaPeriod) => {
        let window = emaData[emaPeriod].slice(timeIndex - windowLength / 2, timeIndex + windowLength / 2);
