...
class BuySellButtons extends React.Component {
	...
	handleNewSymbol(prevProps) {
		...
		if (currentSymbol !== prevSymbol) {
			...
			setTimeout(() => {
				try {
					let close =
						this.props.instrumentType === "commodity"
	...
	async goLong(instrumentType) {
		...
		let { position_size, order_type, order_target_size, order_stop_size, order_limit } = meta;
			...
			<>
				<div className="row flex_center">
					<div className="col-sm-6 flex_center">
						<OrderType value={order_type} setOrder={(e) => dispatch(set_order_type(e.target.value))} />
	...
	return (
		<div className="row flex_center">
			<div className="col-sm-12 flex_center">
				<PositionSize value={position_size} onChange={(e) => dispatch(set_position_size(e.target.value))} />
			</div>
			<div className="col-sm-12 flex_center">
				{order_type !== "Market" && (
					<LimitPrice
...
const PriceBidAsk = ({ currentTick }) => {
	if (!currentTick) {
		...
		let upDownTick = close === currentBidPrice ? "red" : close === currentAskPrice ? "green" : "grey";
...
const BuyBtn = ({ opening_long, goLong, currentTick }) => {
	...
	return (
		<OpenLongPosition style={{ width: "inherit" }} className="btn" disabled={opening_long} onClick={goLong}>
			{opening_long && (
				<>
					{" "}
...
const SellBtn = ({ opening_short, goShort, currentTick }) => {
	...
	return (
		<OpenShortPosition className="btn" style={{ width: "inherit" }} disabled={opening_short} onClick={goShort}>
			{opening_short && (
				<>
					{" "}
					<FontAwesomeIcon icon={faSpinner} spin />
