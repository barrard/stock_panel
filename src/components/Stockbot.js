...
class Stockbot_Page extends React.Component {
    ...
    render() {
        ...
        stochasticMethodTrades.forEach((t) => {
            let { PL } = t;
            let [groupName, symbol, timeframe, buySell] =
                t.stratName.split("_");
            ...
            <div className="container white">
                ...
                <div className="row flex_center">
                    ...
                    <div className="col-sm-12 flex_center">
                        Total ES winners {esWinners.length}/
                        {all_ES_trades.length} ratio{" "}
                        {(esWinners.length / all_ES_trades.length).toFixed(3)}{" "}
                        PL = {this.totalPL(all_ES_trades)}
                    </div>
                    <div className="col-sm-12 flex_center">
                        Total YM winners {ymWinners.length}/
                        {all_YM_trades.length} ratio{" "}
                        {(ymWinners.length / all_YM_trades.length).toFixed(3)}{" "}
                        PL = {this.totalPL(all_YM_trades)}
                    </div>
                    <div className="col-sm-12 flex_center">
                        Total GC winners {gcWinners.length}/
                        {all_GC_trades.length} ratio{" "}
                        {(gcWinners.length / all_GC_trades.length).toFixed(3)}{" "}
                        PL = {this.totalPL(all_GC_trades)}
                    </div>
                    <div className="col-sm-12 flex_center">
                        Total CL winners {clWinners.length}/
                        {all_CL_trades.length} ratio{" "}
                        {(clWinners.length / all_CL_trades.length).toFixed(3)}{" "}
                        PL = {this.totalPL(all_CL_trades)}
                    </div>
                    <div className="col-sm-12 flex_center">
                        Total NQ winners {nqWinners.length}/
                        {all_NQ_trades.length} ratio{" "}
                        {(nqWinners.length / all_NQ_trades.length).toFixed(3)}{" "}
                        PL = {this.totalPL(all_NQ_trades)}
                    </div>
                    <div className="col-sm-12 flex_center">
                        Total RTY winners {rtyWinners.length}/
                        {all_RTY_trades.length} ratio{" "}
                        {(rtyWinners.length / all_RTY_trades.length).toFixed(3)}{" "}
                        PL = {this.totalPL(all_RTY_trades)}
                    </div>

                    <div className="col-sm-12 flex_center">
                        Total 1Min winners {min1Winners.length}/{all1Min.length}{" "}
                        ratio {(min1Winners.length / all1Min.length).toFixed(3)}{" "}
                        PL = {this.totalPL(all1Min)}
                    </div>

                    <div className="col-sm-12 flex_center">
                        Total 5Min winners {min5Winners.length}/{all5Min.length}{" "}
                        ratio {(min5Winners.length / all5Min.length).toFixed(3)}{" "}
                        PL = {this.totalPL(all5Min)}
                    </div>

                    <div className="col-sm-12 flex_center">
                        Total 15Min winners {min15Winners.length}/
                        {all15Min.length} ratio{" "}
                        {(min15Winners.length / all15Min.length).toFixed(3)} PL
                        = {this.totalPL(all15Min)}
                    </div>
                    <div className="col-sm-12 flex_center">
                        Total 30Min winners {min30Winners.length}/
                        {all30Min.length} ratio{" "}
                        {(min30Winners.length / all30Min.length).toFixed(3)} PL
                        = {this.totalPL(all30Min)}
                    </div>
                    <div className="col-sm-12 flex_center">
                        Total 60Min winners {min60Winners.length}/
                        {all60Min.length} ratio{" "}
                        {(min60Winners.length / all60Min.length).toFixed(3)} PL
                        = {this.totalPL(all60Min)}
                    </div>
                    <div className="col-sm-12 flex_center">
                        Total Daily winners {dailyWinners.length}/
                        {allDaily.length} ratio{" "}
