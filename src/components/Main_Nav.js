...
class Main_Nav extends React.Component {
	...
	make_filter_list(search_text) {
		...
		symbol_starts_with = full_list.filter((list_item) => list_item.Ticker.toUpperCase().startsWith(search_text));
		...
		if (filtered_stock_list.length < 100) {
			...
			name_starts_with = full_list.filter((list_item) => list_item.Name.toUpperCase().startsWith(search_text));
		...
		if (filtered_stock_list.length < 100) {
			/* check symbols */
			symbol_list = full_list.filter((list_item) => list_item.Ticker.toUpperCase().includes(search_text));
		...
		if (filtered_stock_list.length < 100) {
			/* check name */
			name_list = full_list.filter((list_item) => list_item.Name.toUpperCase().includes(search_text));
	...
	filtered_stock_list_item(data, index, search) {
		...
		return (
			...
			>
				<span dangerouslySetInnerHTML={this.highlight_search_letters(data.Ticker, search)} />
				{" - "}
				<span dangerouslySetInnerHTML={this.highlight_search_letters(data.Name, search)} />
			</div>
		);
	}
		...
		return (
			<nav className="navbar navbar-dark bg-dark relative ">
				...
				<ul className="nav-bar-links">
					...
					{isLoggedIn && (
						<Logout_Link
							username={this.props.user.user.primary_email.split("@")[0]}
							pathname={pathname}
