...
const { formatData, forwardFill } = require("../../indicators/indicatorHelpers/utils.js");
...
export function updateCommodityTrade(trade, history) {
	let { buyOrSell, order_type, entryPrice, exitPrice, orderStatus, PL, symbol, instrumentType } = trade;
	const toastrOptions = {
	...
	if (orderStatus === "Filled") {
		toastr.success(`${order_type} ${orderStatus} - ${buyOrSell} ${symbol} @${entryPrice}`, toastrOptions);
	} else if (orderStatus === "Closed") {
		toastr.success(
			`${longOrShort} Position in ${symbol} has been ${orderStatus} @${exitPrice}: PnL= ${PL}`,
			toastrOptions
		);
	} else if (orderStatus === "Canceled") {
		toastr.success(
			`${order_type} to  ${buyOrSell} ${symbol} @${entryPrice} has been ${orderStatus} -`,
			toastrOptions
		);
	}
	return {
